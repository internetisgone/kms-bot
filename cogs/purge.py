...
async def purge_channel(channel, dtime, self_msg_id):
    try:
        ...
        print(e) 
        # todo
        # delete task if no access
        # send msg if missing perms
...
class Purge(commands.Cog):
    ...
    async def on_ready(self):
        print(f"{datetime.utcnow()} cogs.purge is ready. latency: {self.bot.latency}")
    ...
    async def kms(self, ctx, usr_input):
        try:
...
async def setup(bot):
    ...
    print(f"{datetime.utcnow()} added cogs.purge")